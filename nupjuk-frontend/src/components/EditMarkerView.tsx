...
import { updateMarker } from "../api/adminMarkers";
...
import type { MarkerCategory, MarkerDetail, MarkerSummary } from "../types/marker";
import { getMarkerId } from "../types/marker";

type EditMarkerViewProps = {
  marker: MarkerDetail;
...
function getInitialParentId(marker: MarkerDetail): string {
  if (!marker.parentId) {
    return "";
  }

  if (typeof marker.parentId === "string") {
    return marker.parentId;
  }

  return marker.parentId._id ?? "";
}

export default function EditMarkerView({
  marker,
  markers,
  onBack,
  onMarkerUpdated
  ...
  const edit = ui.editMarker;
  const m = ui.manager;
  const markerId = getMarkerId(marker);

  const [titleKo, setTitleKo] = useState(marker.titleKo);
  const [category, setCategory] = useState<MarkerCategory>(marker.category);
  const [latitude, setLatitude] = useState(formatCoordinate(marker.latitude));
  const [longitude, setLongitude] = useState(formatCoordinate(marker.longitude));
  const [markdownKo, setMarkdownKo] = useState(marker.markdownKo ?? "");
  const [parentId, setParentId] = useState(getInitialParentId(marker));
  const [activeFrom, setActiveFrom] = useState(toDatetimeLocalValue(marker.activeFrom));
  const [activeUntil, setActiveUntil] = useState(
    toDatetimeLocalValue(marker.activeUntil)
  );
  const existingImageUrl = marker.imageUrl ?? "";
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setStatus({
        kind: "success",
        message: m.errors.markerUpdated(trimmedTitleKo)
      });
      clearNewImage();
      onMarkerUpdated?.();
    } catch (error) {
      let messageText: string = m.errors.markerUpdateFailed;
  ...
  const displayedImageUrl = imagePreviewUrl ?? (existingImageUrl || null);
